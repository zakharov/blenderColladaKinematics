...
void MeshImporter::read_faces(COLLADAFW::Mesh *mesh, Mesh *me, int new_tris)
{
	...
	// allocate faces
	me->totface = mesh->getFacesCount() + new_tris+10000;
	...
	for (i = 0; i < prim_arr.getCount(); i++) {
		...
		*/
		...
		if (type == COLLADAFW::MeshPrimitive::TRIANGLE_FANS) {
		    ...
		    for (unsigned int groupIndex = 0; groupIndex < grouped_vertex; groupIndex++){
		        // Remember first trifan vertex
		        unsigned int firstTriangleVertex = indices[0];
		        unsigned int vertex_count = mp->getGroupedVerticesVertexCount(groupIndex);

                for (unsigned int vertexIndex = 0; vertexIndex < vertex_count-2; vertexIndex++){
                    // Creating a new array , which is copy if indices array
                    unsigned int trifan_indices[4]={firstTriangleVertex, indices[1], indices[2],0};
                    set_face_indices(mface, trifan_indices, false);
                    mface++;
                    indices++;
                 }
                indices +=  2;
